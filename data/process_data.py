...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Academic Data Processing Pipeline")
    parser.add_argument('--input_file', type=str, required=True, help="Path to the input JSONL file")
    parser.add_argument('--processed_dir', type=str, required=True, help="Directory to save processed CSV files")
    args = parser.parse_args()

    print(f"\n🔍 Processing dataset: DATA")
    print(f"📂 Input file: {args.input_file}")

    pipeline = AcademicDataPipeline(args.processed_dir)
    pipeline.run_pipeline(args.input_file, num_workers=8, batch_size=1000000)
